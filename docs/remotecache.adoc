...
Let's wait for this build to be successful! Usually one would need to run all the commands to build the app, build container and then create deploy yaml files etc. Quarkus Openshift extension does this via its maven plugin in a simple 1 liner command as we did here.
